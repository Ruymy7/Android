...
public class LocalPlayerActivity extends ActionBarActivity {
		...
		case PAUSED:
			...
			case LOCAL:
				...
				if (!mCastManager.isConnecting()) {
					...
					mCastManager
							.clearPersistedConnectionInfo(VideoCastManager.CLEAR_SESSION);
				}
				mPlaybackState = PlaybackState.PLAYING;
				startControllersTimer();
				restartTrickplayTimer();
	...
	private void loadRemoteMedia(int position, boolean autoPlay) {
		Log.d(TAG, "loadRemoteMedia() is called. position " + position
				+ " . autoplay " + autoPlay);
	...
	private void setupControlsCallbacks() {
		...
		mSeekbar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress,
					boolean fromUser) {
				mStartText
						.setText(com.google.sample.castcompanionlibrary.utils.Utils
